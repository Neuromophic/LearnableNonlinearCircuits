...
import sys
sys.path.append("/pfs/data5/home/kit/tm/px3192/LearnableNonlinearCircuit/LNC/")
...
for dataset in datasets:
    ...
    for lr_nlc in [1., 0.5, 0.1, 0.05, 0.01, 0.005, 0.001, 0.]:
        config.SetSeed(seed)

        setup = f'dataset:{data_name}_epsilon:{epsilon}_strategy:{strategy}_lr_nlc:{lr_nlc}_seed:{seed}'
        print(setup)
        ...
        if os.path.exists(f'./results_variation/pNN_{setup}'):
            ...
            if strategy == 0:
                optimizer = torch.optim.Adam([{'params':model.GetParam('theta'),'lr':lr_theta},
                                              {'params':model.GetParam('eta'), 'lr':lr_nlc},
                                              {'params':model.GetParam('inv'), 'lr':lr_nlc}])
            ...
            torch.save(model, f'./results_variation/pNN_{setup}')