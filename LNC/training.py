...
def train_nn(nn, train_loader, valid_loader, lossfunction, optimizer, UUID='default'):
    ...
    for epoch in range(10**10):
        for x_train, y_train in train_loader:
            ...
            train_acc, _ = E.BASIC_variation(nn, x_train, y_train)
        ...
        with torch.no_grad():
            for x_valid, y_valid in valid_loader:
                ...
                valid_acc, _ = E.BASIC_variation(nn, x_valid, y_valid)
        ...
        if not epoch % 100:
            print(f'| Epoch: {epoch:-8d} | Train loss: {L_train.item():.5f} | Valid loss: {L_valid.item():.5f} | Train acc: {train_acc:.5f} | Valid acc: {valid_acc:.5f} |')
