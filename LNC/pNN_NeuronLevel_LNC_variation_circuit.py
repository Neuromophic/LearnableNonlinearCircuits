...
class TanhRT(torch.nn.Module):
    ...
    @property
    def RT_extend(self):
        R1 = self.RT_noise[:,0].view(-1,1)
        R2 = self.RT_noise[:,1].view(-1,1)
        R3 = self.RT_noise[:,2].view(-1,1)
        R4 = self.RT_noise[:,3].view(-1,1)
        R5 = self.RT_noise[:,4].view(-1,1)
        W  = self.RT_noise[:,5].view(-1,1)
        L  = self.RT_noise[:,6].view(-1,1)
        k1 = R2 / R1
        k2 = R4 / R3
        k3 = L / W
...
class pNeuron(torch.nn.Module):
    def __init__(self, n_in, N, epsilon):
        super().__init__()
        self.N = N
        self.epsilon = epsilon
    ...
    def forward(self, a): 
        ...
        z = torch.matmul(a_extended, torch.mul(self.w, pt)) + torch.matmul(a_neg, torch.mul(self.w, nt))
        return self.Act(z)
...
class pLayer(torch.nn.Module):
    def __init__(self, n_in, n_out, N, epsilon):
        super().__init__()
        self.N = N
        self.epsilon = epsilon
        self.neurons = torch.nn.ModuleList([pNeuron(n_in, N, epsilon) for i in range(n_out)])
...
class pNN(torch.nn.Module):
    def __init__(self, topology, N, epsilon):
        ...
        for l in range(len(topology)-1):
            self.model.add_module(f'pLayer {l}', pLayer(topology[l], topology[l+1], N, epsilon))
